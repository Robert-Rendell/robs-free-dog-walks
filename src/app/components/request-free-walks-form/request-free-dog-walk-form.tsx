    ...
    return <>
        <div className="request-walk-form">
            ...
            <form id="request-walk-form">
                ...
                <p>Your email</p>
                <input type="email" onChange={(e) => setOwnerEmail(e.target.value)} value={ownerEmail} />

                <p>Dog Breed</p>
                <select onChange={(e) => setDogBreed(e.target.value)} value={dogBreed}>
                    {populateDogBreeds()}
                </select>
                <p>Date / Time slot for walkies</p>
                <input type="datetime-local" onChange={(e) => setBorrowDateTime(e.target.value)} />
                <input className="submit-button" name="submit" type="button" value={formStateLabels[formState]} onClick={(e) => requestWalkFormSubmitted(e)} />
            </form>
        </div></>
}